...
input = """F-F7----F7-7-J--J--7FFL7F-7FFF77JFL|7|-L7--F77FF-FF7..-J7.F---FF7-|-JF7-J-F|-7FF|-FLL--7F-F--.--|77F|F---7-F--F7.L7F77|F----FF-F7FF.F-.L.L.J
...
def create_cell(char) -> Cell:
    cell = Cell()
    if char == '|':
        cell.directions = [Directions.NORTH, Directions.SOUTH]
    if char == '-':
        cell.directions = [Directions.WEST, Directions.EAST]
    if char == 'L':
        cell.directions = [Directions.NORTH, Directions.EAST]
    if char == '7':
        cell.directions = [Directions.SOUTH, Directions.WEST]
    if char == 'J':
        cell.directions = [Directions.NORTH, Directions.WEST]
    if char == 'F':
        cell.directions = [Directions.SOUTH, Directions.EAST]
    if char == 'S':
        cell.directions = [Directions.SOUTH, Directions.EAST, Directions.NORTH, Directions.WEST]
    return cell
...
def get_cell(pos: Vector2, matrix: List[List[int]]):
    try:
...
last_direction = None
# find first connect to start
for direction in [Directions.SOUTH, Directions.NORTH, Directions.EAST, Directions.WEST]:
    if direction in get_cell(pos - direction, grid).directions:
        last_direction = -direction
        break

assert last_direction != None, f"nothing connected to start: {pos}"
...
while pos != start:
    ...
    cell = get_cell(pos, grid)
    next_direction = next(direction for direction in cell.directions if direction != -last_direction)
    if -next_direction in get_cell(pos + next_direction, grid).directions:
        pos += next_direction
        last_direction = next_direction
    else:
        raise ValueError(f"no valid cell from {pos} in direction {next_direction}")
...
for y, line in enumerate(input.splitlines()):
    is_inside = False
    for x, char in enumerate(line):
        if Vector2(x,y) not in valid_pos:
            char = "."
        if char == "|":
        ...
        if char == "." and is_inside:
            inside_cells += 1
        if char in ["J", "L", "7", "F"]:
            if (last_corner == "L" and char == "7") or (last_corner == "F" and char == "J"):
                is_inside = not is_inside
            last_corner = char
